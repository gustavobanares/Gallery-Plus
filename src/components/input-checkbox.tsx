...
export const inputCheckboxInputVariants = tv({
    base:
    `
...
interface InputCheckboxProps extends VariantProps<typeof inputCheckboxInputVariants>, Omit<React.ComponentProps<'input'>, 'size' | 'disabled'> {
}
...
export default function InputCheckbox({variant, size, disabled, className,...props}: InputCheckboxProps) {
    return (
        <label className={inputCheckboxWrapperVariants({className})}>
            <input type="checkbox" {...props} className={inputCheckboxInputVariants({variant, size, disabled})} />
